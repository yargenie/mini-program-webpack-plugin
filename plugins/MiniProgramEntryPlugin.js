...
function _inflateEntries(entries = [], context, dirname, entry) {
  ...
  ['pages', 'usingComponents'].forEach(key => {
    ...
    if (typeof items === 'object') {
      Object.values(items).forEach(item => {
        if (item.startsWith('/')) {
          item = path.resolve(context, item.substring(1));
        }
        return inflateEntries(entries, context, dirname, item);
      });
    }
  });
}

function inflateEntries(entries, context, dirname, entry) {
  entry = path.resolve(dirname, entry);
  if (entry != null && !entries.includes(entry)) {
    entries.push(entry);
    _inflateEntries(entries, context, path.dirname(entry), entry);
  }
}
...
class MiniProgramEntryPlugin {
  ...
  apply(compiler) {
    ...
    inflateEntries(this.entries, context, context, entry);

    // custom-tab-bar单独处理
    const CUSTOM_TAB_BAR_ENTRY = 'custom-tab-bar/index.ts';
    try {
      if (fs.existsSync(path.resolve(context, CUSTOM_TAB_BAR_ENTRY))) {
        inflateEntries(this.entries, context, context, CUSTOM_TAB_BAR_ENTRY);
      }
    } catch (e) {
      console.log('Do not use custom-tab-bar');
    }
...
module.exports = MiniProgramEntryPlugin;
